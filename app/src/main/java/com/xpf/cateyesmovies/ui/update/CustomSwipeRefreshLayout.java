...
public class CustomSwipeRefreshLayout extends ViewGroup implements NestedScrollingParent,
    ...
    public void setProgressView(MaterialProgressDrawable mProgress, int midImageId) {
        this.mProgress = mProgress;
        mCircleView.setImageDrawable(mProgress);
        mCircleView.setBackgroundResource(midImageId);
    }
    ...
    private void setRefreshing(boolean refreshing, final boolean notify) {
        if (mRefreshing != refreshing) {
            ...
            if (mRefreshing) {
                ...
                mTarget.setOverScrollMode(OVER_SCROLL_IF_CONTENT_SCROLLS);
            }
        }
...
//            经过一堆数学处理后的rotation
        ...
        //拖拽时的动画
        mProgress.setProgressRotation(dragPercent, mRefreshing);
...
//                否则继续继续下移
    ...
    private void finishSpinner(float overscrollTop) {
        if (overscrollTop > mTotalDragDistance) {
            ...
            mTarget.animate().translationY(2 * mCircleHeight).setDuration(200).start();
        } else {
            // cancel refresh
            mTarget.animate().translationY(0).setDuration(200).start();
            mRefreshing = false;
